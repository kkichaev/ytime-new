...
class PostTable extends TableAbstract
{
    public function setup(): void
    {
        ...
        $this
            ->model(Post::class)
            ->addHeaderAction(CreateHeaderAction::make()->route('ev-posts.create'))
            ->addActions([
                EditAction::make()->route('ev-posts.edit'),
                DeleteAction::make()->route('ev-posts.destroy'),
            ])
            ->addColumns([
                IdColumn::make(),
                ImageColumn::make(),
                NameColumn::make()->route('ev-posts.edit'),
                FormattedColumn::make('categories_name')
                    ->title(trans('plugins/events-places::posts.categories'))
                    ->width(150)
                    ->orderable(false)
                    ->searchable(false)
                    ->getValueUsing(function (FormattedColumn $column) {
                        $categories = $column
                            ...
                            ->map(function (Category $category) {
                                return Html::link(route('ev-categories.edit', $category->getKey()), $category->name, ['target' => '_blank']);
                            })
                            ->all();
                ...
                FormattedColumn::make('author_id')
                    ->title(trans('plugins/events-places::posts.author'))
                    ->width(150)
                    ->orderable(false)
                    ->searchable(false)
                    ->getValueUsing(function (FormattedColumn $column) {
            ...
            ->addBulkActions([
                DeleteBulkAction::make()->permission('ev-posts.destroy'),
            ])
            ->addBulkChanges([
                NameBulkChange::make(),
                ...
                SelectBulkChange::make()
                    ->name('category')
                    ->title(trans('plugins/events-places::posts.category'))
                    ->searchable()
                ...
                return $query
                    ->with([
                        'categories' => function (BelongsToMany $query): void {
                            $query->select(['ev_categories.id', 'ev_categories.name']);
                        },
                        'author',
                    ])
                    ->select([
