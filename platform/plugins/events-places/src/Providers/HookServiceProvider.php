...
use Botble\EventsPlaces\Services\BlogService;
...
class HookServiceProvider extends ServiceProvider
{
    public function boot(): void
    {
        ...
        $this->app['events']->listen(RenderingAdminBar::class, function (): void {
            AdminBar::registerLink(
                trans('plugins/events-places::posts.post'),
                route('ev-posts.create'),
                'add-new',
                'ev-posts.create'
            );
        });
        ...
        if (function_exists('add_shortcode')) {
            shortcode()
                ->register(
                    ...
                    trans('plugins/events-places::base.short_code_name'),
                    trans('plugins/events-places::base.short_code_description'),
                ...
                ->setAdminConfig(
                    ...
                    function (array $attributes) {
                        ...
                        return ShortcodeForm::createFromArray($attributes)
                            ->withLazyLoading()
                            ->add('paginate', 'number', [
                                'label' => trans('plugins/events-places::base.number_posts_per_page'),
                                'attr' => [
                                    'placeholder' => trans('plugins/events-places::base.number_posts_per_page'),
                                ],
                            ])
                            ->add(
                                'category_ids[]',
        ...
        if (defined('THEME_FRONT_HEADER') && setting('ev_blog_post_schema_enabled', 1)) {
            add_action(BASE_ACTION_PUBLIC_RENDER_SINGLE, function ($screen, $post): void {
                add_filter(THEME_FRONT_HEADER, function ($html) use ($post) {
                    ...
                    $schemaType = setting('ev_blog_post_schema_type', 'NewsArticle');
    ...
    public function addThemeOptions(): void
    {
        ...
        theme_option()
            ->setSection([
                'title' => trans('plugins/events-places::base.settings.title'),
                ...
                'fields' => [
                    [
                        'id' => 'blog_page_id',
                        'type' => 'customSelect',
                        'label' => trans('plugins/events-places::base.blog_page_id'),
                        'attributes' => [
                            'name' => 'blog_page_id',
                            'list' => [0 => trans('plugins/events-places::base.select')] + $pages,
                            'value' => '',
                            'options' => [
                                'class' => 'form-control',
                            ],
                    ...
                    [
                        ...
                        'type' => 'number',
                        'label' => trans('plugins/events-places::base.number_posts_per_page_in_category'),
                        'attributes' => [
                    ...
                    [
                        ...
                        'type' => 'number',
                        'label' => trans('plugins/events-places::base.number_posts_per_page_in_tag'),
                        'attributes' => [
    ...
    {
        if (Auth::guard()->user()->hasPermission('ev-categories.index')) {
            Menu::registerMenuOptions(Category::class, trans('plugins/events-places::categories.menu'));
        }

        if (Auth::guard()->user()->hasPermission('ev-tags.index')) {
            Menu::registerMenuOptions(Tag::class, trans('plugins/events-places::tags.menu'));
        }
    }

    public function registerDashboardWidgets(array $widgets, Collection $widgetSettings): array
    {
        if (! Auth::guard()->user()->hasPermission('ev-posts.index')) {
            return $widgets;
        }
        ...
        return (new DashboardWidgetInstance())
            ->setPermission('ev-posts.index')
            ->setKey('widget_posts_recent')
            ->setTitle(trans('plugins/events-places::posts.widget_posts_recent'))
            ->setIcon('fas fa-edit')
            ->setColor('yellow')
    ...
    public function renderBlogPosts(Shortcode $shortcode): array|string
    {
        ...
        $view = 'plugins/events-places::themes.templates.posts';
    ...
    public function renderBlogPage(?string $content, Page $page): ?string
    {
        if ($page->getKey() == $this->getBlogPageId()) {
            $view = 'plugins/events-places::themes.loop';
    ...
    {
        if ($page->getKey() == $this->getBlogPageId()) {
            $subTitle = Html::tag('span', trans('plugins/events-places::base.blog_page'), ['class' => 'additional-page-name'])
                ->toHtml();
